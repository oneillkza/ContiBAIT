...
plotWCdistribution <- function(strandMatrix, filterThreshold=0.8)
{
	...
	boxplot(matrixHistogram, col=colors, xlab="WC frequency per contig (W-C)/(W+C)", ylab="Number of contigs", main=paste("WC distributions from", nrow(matrixHistogram), "libraries\nwith an average of", round(avContNum, digits=0), "fragments", sep=" ") )
	...
	lines(c(40-length(bob[which(as.numeric(names(bob)) >= filterThreshold)]), 40), c(sum(bob[which(as.numeric(names(bob)) >= filterThreshold)]),sum(bob[which(as.numeric(names(bob)) >= filterThreshold)])), col='green4', lwd=2)
	text(1, sum(bob[which(as.numeric(names(bob)) <= filterThreshold)])+30, pos=4, 'Av. WW contigs', cex=0.7, col='green4')
	text(40, sum(bob[which(as.numeric(names(bob)) >= filterThreshold)])+30, pos=2, 'Av. CC contigs', cex=0.7, col='green4')
